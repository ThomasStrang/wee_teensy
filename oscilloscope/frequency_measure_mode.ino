...
void set_mode_frequency_measure() {
  if(mode==MODE_MEASURE_FREQUENCY) {
...
void generate_signal_frequency_graph() {
  unsigned int first_t=read_signal_start_return_micros();
  int* times_buffer = (int*)malloc(sizeof(int)*number_of_signals_to_measure);
  for(unsigned int i = 0; i < number_of_signals_to_measure; i++) {
    times_buffer[i] = read_signal_start_return_micros();
  }
  for(unsigned int i = number_of_signals_to_measure-1; i >0; i--) {
    times_buffer[i] = times_buffer[i]-times_buffer[i-1];
  }
  times_buffer[0]=times_buffer[0]-first_t;
  for(unsigned int i = 0; i < number_of_signals_to_measure; i++) {
    times_buffer[i]=int(micros_taken_to_frequency_hz(times_buffer[i]));
  }
  send_to_serial_plotter(times_buffer,number_of_signals_to_measure);
  free(times_buffer);
}

void generate_signal_frequency_report_string() {
...
void generate_signal_frequency_output() {
  unsigned int strt=micros();
  if(print_report) {
    generate_signal_frequency_report_string();
  } else {
    generate_signal_frequency_graph();
  }
